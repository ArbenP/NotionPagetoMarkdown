// Check if NotionContentExtractor already exists to prevent multiple declarations
if (typeof window.NotionContentExtractor === 'undefined') {
  class NotionContentExtractor {
    ...
    // Convert a title to a valid markdown anchor
    titleToAnchor(title) {
      return title
        .toLowerCase()
    ...
    extractToMarkdown() {
      ...
      // First collect all the headers and their levels
      this.tableOfContents = [];
      let mainContent = this.processElement(contentArea, 0);
      
      // Format table of contents with proper spacing
      if (this.tableOfContents.length > 0) {
        // Create TOC with proper formatting and ensure each entry is on its own line
        const tocLines = this.tableOfContents.map(entry => {
          return `- [${entry.text}](#${entry.anchor})`;
        });
        
        // Join TOC entries with single newlines and add double newline before content
        const tocSection = tocLines.join('\n');
        
        // Combine TOC with main content, ensuring proper spacing
        this.markdown = tocSection + '\n\n' + mainContent;
      } else {
        this.markdown = mainContent;
      }
    ...
    processElement(element, depth = 0) {
      ...
      if (element.hasAttribute('data-block-id')) {
        ...
        switch (tagName) {
          ...
          case 'h5':
          case 'h6':
            const anchor = this.titleToAnchor(textContent);
          ...
          case 'a':
            if (textContent) {
              result += this.processNotionLink(element);
            } else {
              result += textContent;
            }
            break;
    ...
    processNotionLink(element) {
      const href = element.getAttribute('href');
      const textContent = element.textContent?.trim() || '';
      
      if (!href || !textContent) return textContent;

      // Check if it's an internal Notion link
      if (href.includes('notion.site') || href.startsWith('/')) {
        // For internal links, create an anchor based on the link text
        const anchor = this.titleToAnchor(textContent);
        
        // Store in table of contents if it looks like a heading
        if (this.looksLikeHeader(element)) {
          // Check if we already have this entry to avoid duplicates
          if (!this.tableOfContents.some(entry => entry.anchor === anchor)) {
            this.tableOfContents.push({
              text: textContent,
              anchor: anchor
            });
        ...
        // Return markdown link with proper anchor
        return `[${textContent}](#${anchor})`;
      }
      
      // For external links, keep the original href
        ...
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          while (parent && parent !== element) {
            ...
            if (tag === 'strong' || tag === 'b' || style.includes('font-weight: bold')) {
              ...
              nodeText = this.processNotionLink(parent);
            }
    ...
    cleanupMarkdown(markdown) {
      ...
      cleaned = cleaned
        ...
        .replace(/^#+\s*$/gm, '')
        // Fix links that are missing bullet points
        .replace(/^(\[.*?\]\(#.*?\))$/gm, '- $1')
        // Fix adjacent links (ensure they're on separate lines)
        .replace(/(\]\(#[^)]+\))(\[)/g, '$1\n- $2')
        // Remove any remaining square brackets at the end of lines
        .replace(/\[\]|\(\)|\[?\]$/gm, '')
        // Fix double hash in links
        .replace(/\]\(##+/g, '](#')
        // Fix any remaining spacing issues
        .replace(/\n{3,}/g, '\n\n')
        // Ensure proper spacing before headers
        .replace(/([^\n])\n(###)/g, '$1\n\n$2')
        // Remove any extra blank lines at the start of the document
        .replace(/^\n+/, '')
        // Clean up any remaining link artifacts
        .replace(/\[([^\]]+)\]\(#\)\(#([^)]+)\)/g, '[$1](#$2)')
        // Fix any missing spaces after bullet points
        .replace(/^-\[/gm, '- [');
  ...
  // Export the class for both Node.js and browser environments
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = NotionContentExtractor;
  } else if (typeof window !== 'undefined') {
    window.NotionContentExtractor = NotionContentExtractor;
  }
} 